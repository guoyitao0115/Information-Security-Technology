...
MD_PATH = ROOT / "docs" / "信息安全技术期末报告.md"
PDF_PATH = ROOT / "docs" / "信息安全技术期末报告.pdf"
...
选题方向：选题一，阅读一篇 2021 年及之后 CCF-B 类以上信息安全论文，对核心方案进行改进创新并通过实验验证。

小组成员：郭翊涛（22308043）、贠湘楠（20337264）

分工说明：郭翊涛负责论文阅读整理、检测规则设计、实验结果分析和报告主要撰写；贠湘楠负责改进方案设计、代码实现协助、样例整理、图表检查和报告排版复核。
...
原论文 Characterizing the Security of Github CI Workflows 发表在 31st USENIX Security Symposium，即 USENIX Security 2022。作者包括 Igibek Koishybayev、Aleksandr Nahapetyan、Raima Zachariah、Siddharth Muralee、Bradley Reaves、Alexandros Kapravelos 和 Aravind Machiry。USENIX Security 是信息安全领域的重要会议，满足本课程“2021 年及之后、CCF-B 类以上会议或期刊论文”的选题要求。
...
[^ai]: 本报告和配套代码使用 AI 工具 Codex 辅助完成。使用的大模型为 GPT-5。主要提示词包括：“读取期末考查说明和已有实施方案，完成信息安全技术作业”“实现 GitHub Actions workflow 安全扫描器”“生成实验结果、图表、README 和中文课程报告”。AI 输出主要贡献为：辅助整理课程要求、规划复现与改进方案、生成 Python 代码初稿、根据实际扫描结果组织报告文字和图表说明。最终代码运行结果、实验数据、报告内容和提交文件由小组成员检查确认。
"""
...
def build_pdf(markdown: str, results: list[dict]) -> None:
    ...
    for line in paragraphs:
        ...
        if not stripped:
            ...
            story.append(Paragraph("AI 工具使用说明：" + stripped.removeprefix("[^ai]:").strip(), styles["ChineseSmall"]))
        else:
            story.append(Paragraph(stripped, styles["ChineseBody"]))

    story.insert(42, table_from_results(results, styles))
    for image_name in ["risk_distribution.png", "risk_scores.png", "rule_weights.png"]:
